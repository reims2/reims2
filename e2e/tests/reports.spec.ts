...
test('Download reports', async ({ page }) => {
  ...
  expect((await fs.promises.stat((await inventoryCsv.path()) as string)).size).toBeGreaterThan(1000)
  ...
  expect((await fs.promises.stat((await dispenseCsv.path()) as string)).size).toBeGreaterThan(1000)
})
